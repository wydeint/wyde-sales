...
export default function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      {/* ── Sidebar ─────────────────────────────────── */}
      {/* Desktop: always visible (static); Mobile: slides in from left (fixed overlay) */}
      <div
        className={[
          'fixed inset-y-0 left-0 z-50',           // mobile: overlay
          'lg:static lg:z-auto',                     // desktop: normal flow
          'transition-transform duration-300 ease-in-out',
          sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0',
        ].join(' ')}
      >
        {/* Close button inside sidebar (mobile only) */}
        <div className="lg:hidden absolute top-4 right-[-48px] z-50">
          <button
            onClick={() => setSidebarOpen(false)}
      ...
      <main className="flex-1 overflow-y-auto min-w-0">
        ...
        <div
          className="lg:hidden sticky top-0 z-30 flex items-center gap-3 px-4 h-14 flex-shrink-0"
          style={{

...
// ─── InstRow ──────────────────────────────────────────────
export function InstRow({ inst, job, onDateSaved, onDeleted, onUpdated }: {
  inst: Installment
    ...
    <div className="px-4 py-2.5">
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {inst.is_work_trigger && inst.status !== 'paid' && <span className="ml-1.5 text-[9px] px-1 rounded-[4px] font-semibold" style={{ background: 'color-mix(in srgb, var(--accent) 12%, transparent)', color: 'var(--accent)' }}>เริ่มงาน</span>}
      ...
      {inst.status === 'paid' && (
        <div className="flex gap-1.5 mt-2 ml-7 flex-wrap">
          ...
          <button onClick={toggleSlip} disabled={savingSlip}
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-[6px] text-xs font-semibold active:scale-95"
            style={{ background: slipUrl ? 'rgba(96,165,250,0.12)' : 'var(--hover-bg)', border: `1px solid ${slipUrl ? 'rgba(96,165,250,0.3)' : 'var(--divider)'}`, color: slipUrl ? '#60a5fa' : 'var(--text-3)', opacity: savingSlip ? 0.5 : 1 }}>
            {slipUrl ? <CheckCircle2 size={11} /> : <Circle size={11} />} Slip
          </button>
          <button onClick={toggleReceipt} disabled={savingReceipt}
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-[6px] text-xs font-semibold active:scale-95"
            style={{ background: receiptUrl ? 'rgba(74,222,128,0.12)' : 'var(--hover-bg)', border: `1px solid ${receiptUrl ? 'rgba(74,222,128,0.3)' : 'var(--divider)'}`, color: receiptUrl ? '#4ade80' : 'var(--text-3)', opacity: savingReceipt ? 0.5 : 1 }}>
            {receiptUrl ? <CheckCircle2 size={11} /> : <Circle size={11} />} ใบเสร็จ
          </button>
          <button onClick={copyLine}
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-[6px] text-xs font-semibold active:scale-95"
            style={{ background: copied ? 'rgba(74,222,128,0.15)' : 'rgba(0,185,107,0.08)', border: `1px solid ${copied ? 'rgba(74,222,128,0.4)' : 'rgba(0,185,107,0.25)'}`, color: copied ? '#4ade80' : '#00b96b' }}>
            {copied ? <CheckCircle2 size={11} /> : '💬'} {copied ? 'คัดลอกแล้ว!' : 'LINE'}
          </button>
          <button onClick={deleteInst} disabled={deleting}
            className="flex items-center gap-1 px-2 py-1.5 rounded-[6px] text-xs font-semibold active:scale-95 disabled:opacity-40"
        ...
        <div data-panel className="w-full max-w-[460px] max-h-[90vh] flex flex-col rounded-[20px] shadow-2xl pointer-events-auto"
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            <div className="flex gap-3">
              ...
              {job.order_date && (
                <div className="rounded-[12px] px-4 py-3 flex flex-col justify-center min-w-[120px]" style={{ background: 'var(--hover-bg)' }}>
                  <p className="text-xs" style={{ color: 'var(--text-3)' }}>วันรับจอง</p>
                  <p className="text-sm font-bold mt-0.5" style={{ color: 'var(--text-1)' }}>
                    {new Date(job.order_date).toLocaleDateString('th-TH', { day: 'numeric', month: 'short', year: '2-digit' })}
                  </p>
                </div>
              )}
            </div>
